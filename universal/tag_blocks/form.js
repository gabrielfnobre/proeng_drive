/**
 * Cria um form totalmente neutro, permitindo configurar tudo via atributo style, definir id opcional, inserir qualquer conteúdo (string, HTMLElement, jQuery, array)
 * e inserir o form em um container de sua escolha via cnt_created.
 * 
 * @param {string} cnt_created - id do container onde o form será inserido (opcional, se não passar, retorna apenas o form)
 * @param {object} options - opções para o form:
 *      - content: string|HTMLElement|jQuery|Array - conteúdo(s) a ser(em) inserido(s) dentro do form
 *      - style: string (opcional) - string de CSS inline para customizar o form
 *      - id: string (opcional) - id a ser atribuído ao form
 *      - on_submit: function|string (opcional) - função ou string de função a ser executada no submit
 *      - width_desktop: string (opcional) - largura do form no desktop (ex: '50%', '300px', etc)
 *      - width_mobile: string (opcional) - largura do form no mobile (ex: '90%', '100vw', etc)
 *      - height_desktop: string (opcional) - altura do form no desktop (ex: '40px', 'auto', etc)
 *      - height_mobile: string (opcional) - altura do form no mobile (ex: '60px', 'auto', etc)
 ...
 * @returns {jQuery} - elemento jQuery do form criado
 */
export default function neutral_form(cnt_created = null, options = {}) {
    ...
    const uniqueId = 'neutral-form-' + Date.now() + '-' + Math.floor(Math.random() * 1000000);
    const uniqueClass = `neutral-form-custom-${uniqueId}`;

    const $form = $(`<form class="${uniqueClass}" autocomplete="off"></form>`);
    ...
    // Adiciona o evento de submit se fornecido
    if (on_submit) {
        if (typeof on_submit === 'function') {
            $form.on('submit', function(e) {
                e.preventDefault();
                on_submit.call(this, e);
            });
        } else if (typeof on_submit === 'string') {
            $form.on('submit', function(e) {
                e.preventDefault();
                // eslint-disable-next-line no-new-func
                (new Function('event', on_submit)).call(this, e);
            });
        }
    }
    ...
    // Se cnt_created for passado, insere o form no container correspondente
    if (cnt_created && typeof cnt_created === 'string' && $(`#${cnt_created}`).length > 0) {
        $(`#${cnt_created}`).append($form);
    }
